...
public:
    int minimumDiameterAfterMerge(vector<vector<int>>& edges1, vector<vector<int>>& edges2) {
        ...
        int diameter1 = findDiameter(adjList1, 0, -1).first;  // Start DFS for Tree 1
        int diameter2 = findDiameter(adjList2, 0, -1).first;  // Start DFS for Tree 2
...
private:
    ...
    pair<int, int> findDiameter(vector<vector<int>>& adjList, int node, int parent) {
        ...
        for (int neighbor : adjList[node]) {  // Iterate through neighbors of the current node
            if (neighbor == parent)
