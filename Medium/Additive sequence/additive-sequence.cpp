    ...
    bool isAdditiveSequence(string s) {
        ...
        for(int i=0; i<min(8, n); i++){
            for(int j=i+1; j<min(8, n); j++){
                ...
                if((temp1.size()>1 && temp1[0]=='0') || (temp2.size()>1 && temp2[0]=='0')) continue;
                long long num1 = stoi(temp1);
                long long num2 = stoi(temp2);
                int k = j+1;
                while(k < n){
                    long long sum = num1+num2;
                    string temp3 = to_string(sum);
                    if(k+temp3.size()>n || s.substr(k, temp3.size()) != temp3) break;
                    k += temp3.size();
                    num1 = num2;
                    num2 = sum;
                    if(k==n) return true;
