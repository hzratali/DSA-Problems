...
void getElements(Node *root, vector<int>&inorder, int data){
    ...
    if(root->data != data) inorder.push_back(root->data);
    getElements(root->right, inorder, data);
}
Node *getBalancedTree(vector<int> elements, int height){
    if(elements.size() == 0) return NULL;
    int mid = elements.size() >> 1;
    Node *ans = new Node(elements[mid]);
    ans->height = height;
    if(elements.size() == 1) return ans;
    vector<int> left(elements.begin(), elements.begin() + mid);
    vector<int> right(elements.begin()+mid+1, elements.end());
    ans->left = getBalancedTree(left, height+1);
    ans->right = getBalancedTree(right, height+1);
    return ans;
}
